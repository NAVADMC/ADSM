...
Using the pip in your new Virtual Environment (confirm Virtual Environment Activation with `where pip` or `which pip`), install the following packages.
    ...
    pip install git+https://github.com/BryanHurst/django-productionserver.git@v1.0.5r4  # Note, this currently throws permission errors but actually works. Fix in Pip dev version.
