...
When releaseing a Beta compile:
  ...
  - Bump the version in `ADSM/__init__.py` and in `package.json`. See the [Version Guide](/NAVADMC/ADSM/wiki/Version-Guide) for the meaning of the four parts of the version number.
...
When releasing a Production compile:
  ...
  - Bump the version in `ADSM/__init__.py`, in `package.json` and in `installer_windows.nsi`. See the [Version Guide](/NAVADMC/ADSM/wiki/Version-Guide) for the meaning of the four parts of the version number.
  - Build (with sourced python) `python setup.py build`
