...
Using the pip in your new Virtual Environment (confirm Virtual Environment Activation with `where pip` or `which pip`), install the following packages.
    ...
    pip install git+git://github.com/BryanHurst/django-productionserver.git@1.0.1R3  # Note, this currently throws permission errors but actually works. Fix in Pip dev version.
