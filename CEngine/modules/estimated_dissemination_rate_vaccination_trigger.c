...
#define handle_vaccination_terminated_event estimated_dissemination_rate_vaccination_trigger_handle_vaccination_terminated_event
      ...
      if (local_data->last_count > 0)
        {
          ...
          if (local_data->trigger_active && (edr >= local_data->threshold))
            {
              #if DEBUG
                                     ...
                                     EVT_vaccination_terminated_event_t *event)
{
  ...
  local_data->last_count = local_data->current_count = 0;
     ...
     ZON_zone_list_t * zones, gpointer user_data, GError **error)
{
  ...
  sql = g_strdup_printf ("SELECT id,ratio,days,restart_only "
