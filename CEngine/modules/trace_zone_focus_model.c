...
set_params (void *data, GHashTable *dict)
{
  ...
  errno = 0;
  tmp = strtol (g_hash_table_lookup (dict, "direct_trace_is_a_zone_trigger"), NULL, /* base */ 10);
  ...
  errno = 0;
  tmp = strtol (g_hash_table_lookup (dict, "indirect_trace_is_a_zone_trigger"), NULL, /* base */ 10);
  g_assert (errno != ERANGE && errno != EINVAL);
  g_assert (tmp == 0 || tmp == 1);
  if (tmp == 1)
    {
     ...
     ZON_zone_list_t * zones)
{
  ...
  sqlite3_exec_dict (params,
                     "SELECT prodtype.name AS prodtype,direct_trace_is_a_zone_trigger,indirect_trace_is_a_zone_trigger "
