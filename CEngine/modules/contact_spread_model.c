     ...
     ZON_zone_list_t * zones)
{
  ...
  sqlite3_exec (params,
                "SELECT zone.name,prodtype.name,\"direct\",zone_direct_movement_id FROM ScenarioCreator_zone zone,ScenarioCreator_productiontype prodtype,ScenarioCreator_zoneeffect pairing WHERE zone.id=pairing.zone_id AND prodtype.id=pairing.production_type_id AND zone_direct_movement_id IS NOT NULL UNION SELECT zone.name,prodtype.name,\"indirect\",zone_indirect_movement_id FROM ScenarioCreator_zone zone,ScenarioCreator_productiontype prodtype,ScenarioCreator_zoneeffect pairing WHERE zone.id=pairing.zone_id AND prodtype.id=pairing.production_type_id AND zone_indirect_movement_id IS NOT NULL",
