     ...
     ZON_zone_list_t * zones)
{
  ...
  sqlite3_exec (params,
                "SELECT zone.name,prodtype.name,zone_detection_multiplier FROM ScenarioCreator_zone zone,ScenarioCreator_productiontype prodtype,ScenarioCreator_zoneeffect pairing WHERE zone.id=pairing.zone_id AND prodtype.id=pairing.production_type_id AND zone_detection_multiplier IS NOT NULL",
