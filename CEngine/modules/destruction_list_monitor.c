 ...
 */
void
handle_commitment_to_destroy_event (struct spreadmodel_model_t_ *self,
  ...
  if (g_hash_table_lookup (local_data->status, unit) == NULL)
    {
      ...
      if (NULL != spreadmodel_queue_unit_for_destruction)
        {
          spreadmodel_queue_unit_for_destruction (unit->index);
        }
